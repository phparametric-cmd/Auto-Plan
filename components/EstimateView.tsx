...
const EstimateView: React.FC<EstimateViewProps> = ({ house, estimateHtml }) => {
  const t = getTranslation(house.lang).estimateView;
    ...
    <div id="estimate-doc-root" className="flex flex-col gap-12 text-slate-900 p-16 font-sans bg-white pb-24" style={{ width: '850px' }}>
      <div className="flex justify-between items-start border-b border-slate-200 pb-10 mt-8">
        ...
        <div className="flex flex-col items-end gap-4 text-right">
          <h1 className="text-4xl font-light uppercase tracking-[0.4em] leading-tight text-slate-900">{t.title1}<br/><span className="font-black">{t.title2}</span></h1>
          <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.5em]">{t.projectNo} {house.name}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-16">
        <div className="col-span-6 space-y-12">
          <div className="space-y-2">
            <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.3em] mb-4">{t.customer}</p>
          ...
          <div className="space-y-6">
            <h3 className="text-[9px] font-black uppercase text-slate-300 tracking-[0.3em]">{t.objectParams}</h3>
            <div className="space-y-4">
              <div className="flex justify-between border-b border-slate-50 py-3 text-[13px] font-medium">
                <span className="text-slate-400">{t.totalHouseArea}</span>
                <span className="text-slate-900 font-bold">{Math.round(house.houseWidth * house.houseLength * house.floors)} {getTranslation(house.lang).sqm}</span>
              </div>
              <div className="flex justify-between border-b border-slate-50 py-3 text-[13px] font-medium">
                <span className="text-slate-400">{t.plotArea}</span>
                <span className="text-slate-900 font-bold">{(house.plotWidth * house.plotLength / 100).toFixed(1)} {getTranslation(house.lang).sot}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-6 space-y-6">
          <h3 className="text-[9px] font-black text-slate-300 uppercase tracking-[0.3em]">{t.styleVisualization}</h3>
          <div className="rounded-[48px] overflow-hidden border border-slate-100 shadow-xl aspect-[4/3] flex items-center justify-center bg-slate-50 grayscale-[0.3] hover:grayscale-0 transition-all duration-700">
            {house.styleImageUrl ? (
              ...
              <p className="text-[10px] font-black text-slate-300 uppercase">{t.noImage}</p>
            )}
          </div>
        </div>
      </div>
